...
public class PlayerEvents {
    ...
    public static void regenerateMana(TickEvent.PlayerTickEvent event) {
        ...
        if (player.isAlive()) {
            ...
            if (player.getPersistentData().getInt("regentick") >= 60 && playercap.returnMaxMana() != 0) {
                float mana = playercap.returnMana();
                float maxMana = playercap.returnMaxMana();
                float regenMana = playercap.returnRegenMana();
    ...
    public static void cooldowns(TickEvent.PlayerTickEvent event){
        ...
        if (player.isAlive()) {
                ...
                NetworkLoader.INSTANCE.send(PacketDistributor.PLAYER.with(() -> (ServerPlayerEntity) player), new PacketKeybindCD(1, playercap.returnKeybind1CD(), true));
            }
            if (playercap.returnKeybind2CD() > 0) {
                playercap.setKeybind2CD(playercap.returnKeybind2CD() - 1);
                NetworkLoader.INSTANCE.send(PacketDistributor.PLAYER.with(() -> (ServerPlayerEntity) player), new PacketKeybindCD(2, playercap.returnKeybind2CD(), true));
            }
            if (playercap.returnKeybind3CD() > 0) {
                playercap.setKeybind3CD(playercap.returnKeybind3CD() - 1);
                NetworkLoader.INSTANCE.send(PacketDistributor.PLAYER.with(() -> (ServerPlayerEntity) player), new PacketKeybindCD(3, playercap.returnKeybind3CD(), true));
            }
            if (playercap.returnKeybind4CD() > 0) {
                playercap.setKeybind4CD(playercap.returnKeybind4CD() - 1);
                NetworkLoader.INSTANCE.send(PacketDistributor.PLAYER.with(() -> (ServerPlayerEntity) player), new PacketKeybindCD(4, playercap.returnKeybind4CD(), true));
            }
            if (playercap.returnKeybind5CD() > 0) {
                playercap.setKeybind5CD(playercap.returnKeybind5CD() - 1);
                NetworkLoader.INSTANCE.send(PacketDistributor.PLAYER.with(() -> (ServerPlayerEntity) player), new PacketKeybindCD(5, playercap.returnKeybind5CD(), true));
            }
            if (playercap.returnKeybind6CD() > 0) {
                playercap.setKeybind6CD(playercap.returnKeybind6CD() - 1);
                NetworkLoader.INSTANCE.send(PacketDistributor.PLAYER.with(() -> (ServerPlayerEntity) player), new PacketKeybindCD(6, playercap.returnKeybind6CD(), true));
            }
            if (playercap.returnKeybind7CD() > 0) {
                playercap.setKeybind7CD(playercap.returnKeybind7CD() - 1);
                NetworkLoader.INSTANCE.send(PacketDistributor.PLAYER.with(() -> (ServerPlayerEntity) player), new PacketKeybindCD(7, playercap.returnKeybind7CD(), true));
            }
            if (playercap.returnKeybind8CD() > 0) {
                playercap.setKeybind8CD(playercap.returnKeybind8CD() - 1);
                NetworkLoader.INSTANCE.send(PacketDistributor.PLAYER.with(() -> (ServerPlayerEntity) player), new PacketKeybindCD(8, playercap.returnKeybind8CD(), true));
            }
            if (playercap.returnKeybind9CD() > 0) {
                playercap.setKeybind9CD(playercap.returnKeybind9CD() - 1);
                NetworkLoader.INSTANCE.send(PacketDistributor.PLAYER.with(() -> (ServerPlayerEntity) player), new PacketKeybindCD(9, playercap.returnKeybind9CD(), true));
            }
        }
    }
    ...
    public static void PlayerJoinedWorld(EntityJoinWorldEvent event) {
        ...
        NetworkLoader.INSTANCE.send(PacketDistributor.PLAYER.with(() -> (ServerPlayerEntity) player), new PacketSetGrimoire(true, true));


        if (!player_cap.joinWorld()) {

            player_cap.setKeybind1CD(0);
            NetworkLoader.INSTANCE.send(PacketDistributor.PLAYER.with(() -> (ServerPlayerEntity) player), new PacketKeybindCD(1, 0, true));
            player_cap.setKeybind2CD(0);
            NetworkLoader.INSTANCE.send(PacketDistributor.PLAYER.with(() -> (ServerPlayerEntity) player), new PacketKeybindCD(2, 0, true));
            player_cap.setKeybind3CD(0);
            NetworkLoader.INSTANCE.send(PacketDistributor.PLAYER.with(() -> (ServerPlayerEntity) player), new PacketKeybindCD(3, 0, true));
            player_cap.setKeybind4CD(0);
            NetworkLoader.INSTANCE.send(PacketDistributor.PLAYER.with(() -> (ServerPlayerEntity) player), new PacketKeybindCD(4, 0, true));
            player_cap.setKeybind5CD(0);
            NetworkLoader.INSTANCE.send(PacketDistributor.PLAYER.with(() -> (ServerPlayerEntity) player), new PacketKeybindCD(5, 0, true));
            player_cap.setKeybind6CD(0);
            NetworkLoader.INSTANCE.send(PacketDistributor.PLAYER.with(() -> (ServerPlayerEntity) player), new PacketKeybindCD(6, 0, true));
            player_cap.setKeybind7CD(0);
            NetworkLoader.INSTANCE.send(PacketDistributor.PLAYER.with(() -> (ServerPlayerEntity) player), new PacketKeybindCD(7, 0, true));
            player_cap.setKeybind8CD(0);
            NetworkLoader.INSTANCE.send(PacketDistributor.PLAYER.with(() -> (ServerPlayerEntity) player), new PacketKeybindCD(8, 0, true));
            player_cap.setKeybind9CD(0);
            NetworkLoader.INSTANCE.send(PacketDistributor.PLAYER.with(() -> (ServerPlayerEntity) player), new PacketKeybindCD(9, 0, true));
    ...
    public static void manaRuns(TickEvent.PlayerTickEvent event){
        ...
        if(Arrays.stream(player_cap.returnMagicAttribute().getGrimoireTextures().toArray(new String[0])).noneMatch(
            ...
            NetworkLoader.INSTANCE.send(PacketDistributor.PLAYER.with(() -> (ServerPlayerEntity) event.player), new PacketSetGrimoireTexture(randomGrimoire, true));
        }
        ...
        if(entity instanceof PlayerEntity){
            ...
            if (damageSrc.isBypassMagic()) {
                ...
                if (player_cap.returnManaSkinToggled() && damageSrc != DamageSource.OUT_OF_WORLD) {
                    int i = (player_cap.returnMagicLevel() / 4);
