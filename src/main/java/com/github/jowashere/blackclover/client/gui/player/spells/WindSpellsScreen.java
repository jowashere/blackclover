...
public class WindSpellsScreen extends AbstractSpellScreen{
    ...
    public WindSpellsScreen() {
        super(new TranslationTextComponent("gui." + Main.MODID + ".title.yunowindspells"));
    ...
    public void registerSpells(IPlayerHandler playerCapability) {
        ...
        for (BCMSpell spell : this.spells) {
            addButton(new GuiButtonSpell(this.guiLeft + x, this.guiTop + y, spell.getU(), spell.getV(), spell.getCorrelatedPlugin().getPluginId() + "." + spell.getName(), false, spell.getResourceLocationForGUI(), spell.getPress()));
            if (x == 90) {
                x = -90;
                y += 20;
            }
