...
public class DarkParticleData implements IParticleData{
    public DarkParticleData(Color tint, double diameter) {
        this.tint = tint;
    ...
    @Nonnull
    @Override
    public ParticleType<DarkParticleData> getType() {
        return StartupCommonParticle.darkParticleType;
    }
    ...
    public static final Codec<DarkParticleData> CODEC = RecordCodecBuilder.create(
            instance -> instance.group(
                    Codec.INT.fieldOf("tint").forGetter(d -> d.tint.getRGB()),
                    Codec.DOUBLE.fieldOf("diameter").forGetter(d -> d.diameter)
            ).apply(instance, DarkParticleData::new)
    );

    private DarkParticleData(int tintRGB, double diameter) {
    ...
    public static final IParticleData.IDeserializer<DarkParticleData> DESERIALIZER = new IParticleData.IDeserializer<DarkParticleData>() {

        // parse the parameters for this particle from a /particle command
        @Nonnull
        @Override
        public DarkParticleData fromCommand(ParticleType<DarkParticleData> type, com.mojang.brigadier.StringReader reader) throws CommandSyntaxException {
            reader.expect(' ');
            ...
            return new DarkParticleData(color, diameter);
        }

        // read the particle information from a PacketBuffer after the client has received it from the server
        @Override
        public DarkParticleData fromNetwork(@Nonnull ParticleType<DarkParticleData> type, PacketBuffer buf) {
