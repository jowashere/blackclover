...
public class ThunderFiendStrike extends AbstractSpell {

    public ThunderFiendStrike()
    {
        super("thunder_fiend_strike", AttributeInit.LIGHTNING);

        this.setManaCost(45F);
        this.setCooldown(140);
    ...
    private void action(LivingEntity caster, float manaIn) {
        if (!caster.level.isClientSide) {
            ...
            BCMHelper.waitThen(caster.level, 15, () ->
            {
                Vector3d speed = BCMHelper.Propulsion(caster, 3, 3);
                caster.setDeltaMovement(speed.x, 0.3, speed.z);
                caster.hurtMarked = true;
            ...
            BCMHelper.waitThen(caster.level, 30, () ->
            {
                Vector3d speed = BCMHelper.Propulsion(caster, 3, 3);
                caster.setDeltaMovement(speed.x, 0.3, speed.z);
                caster.hurtMarked = true;
